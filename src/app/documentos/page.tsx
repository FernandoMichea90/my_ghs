    ...
    <div className='flex-1 min-h-screen items-center justify-between text-gray-500 py-24 md:py-24 px-10 md:px-24'>
      {loading ? (
        <div className="flex flex-col items-center justify-center">
          <p className="text-primary font-semibold mb-4">Cargando datos...</p>
          {/* Skeleton loading */}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 my-3">
            {home.tags.map((tag, index) => (
              <a href={'/documentos?tag=' + tag} key={index}>
                <span className="bg-primary hover:bg-gray-900 text-white font-bold px-3 py-1 rounded-full text-sm">{tag}</span>
              </a>
            ))}
          </div>
